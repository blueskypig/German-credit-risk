...
%% classification
disp('Minimum-Distance-Classification')
...
for tindex=1:foldnum
    ...
    for T=1:ctimes
        ...
        for fnum=1:size(features,2)
            ...
            for i=1:foldnum 
                ...
                %% classifier
                centroids=zeros(2,fnum);
                for ci=1:2
                    centroids(ci,:)=mean(trfeatures(find(trlabels==ci),:));
                end
                distance=pdist2(vfeatures,centroids);
                [~, pred_labels] = min(distance, [], 2);
                %% evaluation
    ...
    %% classifier
     centroids=zeros(2,maxindex);
     for ci=1:2
        centroids(ci,:)=mean(traindata(find(trainlabels==ci),:));
     end
     distance=pdist2(testdata,centroids);
     [~, pred_labels] = min(distance, [], 2);
     %% evaluation
